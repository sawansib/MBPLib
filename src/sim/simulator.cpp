...
#include "tage_sc_l_sim_only.hpp"
...
namespace mbp {
  ...
  json Simulate(Predictor* branchPredictor, const SimArgs& args) {
    ...
    struct BranchInfo {
      ...
      std::unordered_map<Branch::PredictorSrc, uint64_t> predictionCount;
      std::unordered_map<Branch::PredictorSrc, uint64_t> misspredictionCount;
    };
    ...
    while ((instrNum = trace.nextBranch(b)) < stopAtInstr) {
      if (b.isConditional()) {
        ...
        if (instrNum >= warmupInstrs) {
          ...
          auto predictionType = branchPredictor->getPredictionType();

          //std::cout << "prediction " << predictionType << std::endl;

          // Increment the count for each prediction type
          switch (predictionType) {
          case Branch::PredictorSrc::AltBank:
            branchInfo[b.ip()].predictionCount[Branch::PredictorSrc::AltBank]++;
            if (mispredicted)
              branchInfo[b.ip()].misspredictionCount[Branch::PredictorSrc::AltBank]++;
            break;
          case Branch::PredictorSrc::HitBank:
            branchInfo[b.ip()].predictionCount[Branch::PredictorSrc::HitBank]++;
            if (mispredicted)
              branchInfo[b.ip()].misspredictionCount[Branch::PredictorSrc::HitBank]++;
            break;
          case Branch::PredictorSrc::LP:
            branchInfo[b.ip()].predictionCount[Branch::PredictorSrc::LP]++;
            if (mispredicted)
              branchInfo[b.ip()].misspredictionCount[Branch::PredictorSrc::LP]++;
            break;
          case Branch::PredictorSrc::SC:
            branchInfo[b.ip()].predictionCount[Branch::PredictorSrc::SC]++;
            if (mispredicted)
              branchInfo[b.ip()].misspredictionCount[Branch::PredictorSrc::SC]++;
            break;
          case Branch::PredictorSrc::BiModal:
            branchInfo[b.ip()].predictionCount[Branch::PredictorSrc::BiModal]++;
            if (mispredicted)
              branchInfo[b.ip()].misspredictionCount[Branch::PredictorSrc::BiModal]++;
            break;
          default:
            break;
          }
    ...
    for (auto& [ip, inf] : mostFailed) {
      json j{
          ...
          {"AltBank", {
              {"predictions", inf.predictionCount[Branch::PredictorSrc::AltBank]},
              {"misspredictions", inf.misspredictionCount[Branch::PredictorSrc::AltBank]}
          }},

          {"HitBank", {
              {"predictions", inf.predictionCount[Branch::PredictorSrc::HitBank]},
              {"misspredictions", inf.misspredictionCount[Branch::PredictorSrc::HitBank]}
          }},
          {"LP", {
              {"predictions", inf.predictionCount[Branch::PredictorSrc::LP]},
              {"misspredictions", inf.misspredictionCount[Branch::PredictorSrc::LP]}
          }},
          {"SC", {
              {"predictions", inf.predictionCount[Branch::PredictorSrc::SC]},
              {"misspredictions", inf.misspredictionCount[Branch::PredictorSrc::SC]}
          }},
          {"BiModal", {
              {"predictions", inf.predictionCount[Branch::PredictorSrc::BiModal]},
              {"misspredictions", inf.misspredictionCount[Branch::PredictorSrc::BiModal]}
          }},
        {"Altbank_miss_rate", 100.0 * inf.misspredictionCount[Branch::PredictorSrc::AltBank] / inf.predictionCount[Branch::PredictorSrc::AltBank]},
        {"Hitbank_miss_rate", 100.0 * inf.misspredictionCount[Branch::PredictorSrc::HitBank] / inf.predictionCount[Branch::PredictorSrc::HitBank]},
        {"LP_miss_rate", 100.0 * inf.misspredictionCount[Branch::PredictorSrc::LP] / inf.predictionCount[Branch::PredictorSrc::LP]},
        {"SC_miss_rate", 100.0 * inf.misspredictionCount[Branch::PredictorSrc::SC] / inf.predictionCount[Branch::PredictorSrc::SC]},
        {"BiModal_miss_rate", 100.0 * inf.misspredictionCount[Branch::PredictorSrc::BiModal] / inf.predictionCount[Branch::PredictorSrc::BiModal]}
      };
